...
class LMX2820:
    ...
    def __init__(self, spi, f_in=10e6, f_outa=10e9):
    ...
    def set_fout(self, a, b=None):
        ...
        print(f"VCO frequency: {f_vco} VCO no: {self.get_vco(f_vco)}")
        print(f"PLL: int n: {self._pll_n} num: {self._pll_num} den: {self._pll_den}")
        print(f"Freq: {self.f_pfd * (self._pll_n + self._pll_num / self._pll_den)}")
    ...
    def init_regs_to_reset(self):
        ...
        self._cal_clk_div = 0x3
        self._instacal_dly = 0x1F4
    ...
    R[22] = lambda: ((self._vco_sel << 13) | (0x02 << 8) | (self._vco_capctl))
...
LO_HI = LMX2820(SPI("/dev/spidev1.6", 0, 1000000))
LO_LO = LMX2820(SPI("/dev/spidev1.7", 0, 1000000))
